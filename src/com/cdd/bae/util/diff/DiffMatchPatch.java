...
/*
	Origin:
		https://raw.githubusercontent.com/google/diff-match-patch
		
	Modified to fit BAE style.
*/
...
/*
	Methods for generating a patch that can be used to convert one string to another.
*/
public class DiffMatchPatch
{
	...
	public static class Patch
	{
		...
		public String toString()
		{
			...
			if (length1 == 0) coords1 = start1 + ",0";
			else if (length1 == 1) coords1 = Integer.toString(start1 + 1);
			else coords1 = (start1 + 1) + "," + length1;

			if (length2 == 0) coords2 = start2 + ",0";
			else if (length2 == 1) coords2 = Integer.toString(start2 + 1);
			else coords2 = (start2 + 1) + "," + length2;
	 ...
	 */
	public static LinkedList<Patch> patchMake(String text1, String text2)
	{
		if (text1 == null || text2 == null) throw new IllegalArgumentException("Null inputs.");
	 ...
	 * @return LinkedList of Patch objects.
	 */
	public static LinkedList<Patch> patchMake(LinkedList<Diff> diffs)
	{
		if (diffs == null) throw new IllegalArgumentException("Null inputs.");
	 ...
	 * @return LinkedList of Patch objects.
	 */
	public static LinkedList<Patch> patchMake(String text1, LinkedList<Diff> diffs)
	{
		if (text1 == null || diffs == null) throw new IllegalArgumentException("Null inputs.");
	 ...
	 */
	public static LinkedList<Patch> patchFromText(String textline) throws IllegalArgumentException
	{
		LinkedList<Patch> patches = new LinkedList<Patch>();
	...
	/**
	 * Merge a set of patches onto the text, and returns the transformed text result.
	 * @param patches Array of Patch objects
	 * @param text Old text.
	 * @return Transformed text.
	 */
	public static String patchApplyText(LinkedList<Patch> patches, String text)
	{
		return (String)patchApply(patches, text)[0];
	}
	 ...
	 *      boolean values.
	 */
	public static Object[] patchApply(LinkedList<Patch> patches, String text)
	{
		...
		for (Patch aPatch : patches)
		{
			...
			if (text1.length() > MATCH_MAXBITS)
			{
				...
				// a monster delete.
				start_loc = matchMain(text, text1.substring(0, MATCH_MAXBITS), expected_loc);
				if (start_loc != -1)
				{
					end_loc = matchMain(text, text1.substring(text1.length() - MATCH_MAXBITS), expected_loc + text1.length() - MATCH_MAXBITS);
			...
			else start_loc = matchMain(text, text1, expected_loc);
					...
					else
					{
						...
						for (Diff aDiff : aPatch.diffs)
						{
							if (aDiff.operation != Operation.EQUAL)
							{
								...
								{
									// Deletion
									text = text.substring(0, start_loc + index2) + text.substring(start_loc + diffXIndex(diffs, index1 + aDiff.text.length()));
								}
							}
	 ...
	 * @return Linked List of Diff objects.
	 */
	private static LinkedList<Diff> diffMain(String text1, String text2, boolean checklines, long deadline)
	{
	 ...
	 * @return Linked List of Diff objects.
	 */
	private static LinkedList<Diff> diffLineMode(String text1, String text2, long deadline)
	{
	 ...
	 */
	private static LinkedList<Diff> diffBisect(String text1, String text2, long deadline)
	{
	 ...
	 * @return LinkedList of Diff objects.
	 */
	private static LinkedList<Diff> diffBisectSplit(String text1, String text2, int x, int y, long deadline)
	{
	 ...
	 */
	private static LinesToCharsResult diffLinesToChars(String text1, String text2)
	{
	 ...
	 */
	private static String diffLinesToCharsMunge(String text, List<String> lineArray, Map<String, Integer> lineHash)
	{
		int lineStart = 0;
		int lineEnd = -1;
		String line;
	 ...
	 * @param lineArray List of unique strings.
	 */
	private static void diffCharsToLines(LinkedList<Diff> diffs, List<String> lineArray)
	{
		StringBuilder text;
		for (Diff diff : diffs)
		{
	...
	private static String[] diffHalfMatch(String text1, String text2)
	{
		...
		else if (hm2 == null) hm = hm1;
		else if (hm1 == null) hm = hm2;
		else
		{
	 ...
	 * @param diffs LinkedList of Diff objects.
	 */
	private static void diffCleanupSemantic(LinkedList<Diff> diffs)
	{
	 ...
	 * @param diffs LinkedList of Diff objects.
	 */
	private static void diffCleanupSemanticLossless(LinkedList<Diff> diffs)
	{
	 ...
	 * @param diffs LinkedList of Diff objects.
	 */
	private static void diffCleanupEfficiency(LinkedList<Diff> diffs)
	{
		if (diffs.isEmpty()) return;
	 ...
	 * @param diffs LinkedList of Diff objects.
	 */
	private static void diffCleanupMerge(LinkedList<Diff> diffs)
	{
	 ...
	 */
	private static int diffXIndex(LinkedList<Diff> diffs, int loc)
	{
		int chars1 = 0;
		int chars2 = 0;
		int last_chars1 = 0;
	 ...
	 */
	private static String diffPrettyHTML(LinkedList<Diff> diffs)
	{
	 ...
	 * @return Source text.
	 */
	private static String diffText1(LinkedList<Diff> diffs)
	{
		StringBuilder text = new StringBuilder();
		for (Diff aDiff : diffs)
		{
			if (aDiff.operation != Operation.INSERT) text.append(aDiff.text);
		}
		return text.toString();
	}
	 ...
	 */
	private static String diffText2(LinkedList<Diff> diffs)
	{
		StringBuilder text = new StringBuilder();
		for (Diff aDiff : diffs)
		{
			if (aDiff.operation != Operation.DELETE) text.append(aDiff.text);
		}
		return text.toString();
	}
	 ...
	 */
	private static int diffLevenshtein(LinkedList<Diff> diffs)
	{
		int levenshtein = 0;
		int insertions = 0;
		int deletions = 0;
	 ...
	 * @return Delta text.
	 */
	private static String diffToDelta(LinkedList<Diff> diffs)
	{
		StringBuilder text = new StringBuilder();
		for (Diff aDiff : diffs)
		{
	 ...
	 * @throws IllegalArgumentException If invalid input.
	 */
	private static LinkedList<Diff> diffFromDelta(String text1, String delta) throws IllegalArgumentException
	{
		LinkedList<Diff> diffs = new LinkedList<Diff>();
	 ...
	 */
	private static Map<Character, Integer> matchAlphabet(String pattern)
	{
	 ...
	 */
	private static LinkedList<Patch> patchDeepCopy(LinkedList<Patch> patches)
	{
	 ...
	 */
	private static String patchAddPadding(LinkedList<Patch> patches)
	{
	 ...
	 */
	private static void patchSplitMax(LinkedList<Patch> patches)
	{
	...
	private static String unescapeForEncodeUriCompatability(String str)
	{
		return str.replace("%21", "!").replace("%7E", "~").replace("%27", "'").replace("%28", "(").replace("%29", ")").replace("%3B", ";").replace("%2F", "/")
						.replace("%3F", "?").replace("%3A", ":").replace("%40", "@").replace("%26", "&").replace("%3D", "=").replace("%2B", "+")
