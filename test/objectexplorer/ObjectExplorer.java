 ...
 * primitive value, excluding static fields from the traversal. The traversal is
 * controlled by a user-supplied {@link ObjectVisitor}, which decides for each
 * explored path whether to continue exploration of that path, and it can also
...
public class ObjectExplorer
{
	 ...
	 *
	 * @param <T>
	 *          the type of the value obtained (after the traversal) by the
	 *          ObjectVisitor
	 * @param rootObject
	 *          an object to be recursively explored
	 * @param visitor
	 *          a visitor that is notified for each explored path and decides
	 *          whether to continue exploration of that path, and constructs a
	 ...
	 *
	 * <p>
	 * The {@code features} further customizes the exploration behavior. In
	 * particular:
	 * <ul>
	 * <li>If {@link Feature#VISIT_PRIMITIVES} is contained in features, the visitor
	 * will also be notified about exploration of primitive values.
	 ...
	 * </ul>
	 * In both cases above, the return value of {@link ObjectVisitor#visit(Chain)}
	 * is ignored, since neither primitive values or {@code null} can be further
	 * explored.
	 *
	 * @param <T>
	 *          the type of the value obtained (after the traversal) by the
	 ...
	 *          return value at the end of the exploration
	 * @param features
	 *          a set of desired features that the object exploration should have
	 * @return whatever value is returned by the visitor at the end of the traversal
	 * @see ObjectVisitor
	 */
