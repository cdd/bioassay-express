/*
	Code taken from https://github.com/DimitrisAndreou/memory-measurer
 */
...
public class ObjectGraphMeasurer
{
	...
	public static class Footprint
	{
		...
		private static final ImmutableSet<Class<?>> primitiveTypes = ImmutableSet.<Class<?>>of(boolean.class, byte.class,
				char.class, short.class, int.class, float.class, long.class, double.class);

		/**
		 * Constructs a Footprint, by specifying the number of objects, references, and
		 * primitives (represented as a {@link Multiset}).
		 *
		 * @param objects
		 ...
		 * @param primitives
		 *          the number of primitives (represented by the respective primitive
		 *          classes, e.g. {@code int.class} etc)
		 */
		public Footprint(int objects, int references, Multiset<Class<?>> primitives)
		{
			...
			Preconditions.checkArgument(primitiveTypes.containsAll(primitives.elementSet()), "Unexpected primitive type");
		...
		/**
		 * Returns the number of primitives of this footprint (represented by the
		 * respective primitive classes, {@literal e.g.} {@code int.class} etc).
		 */
		...
		{
			return MoreObjects.toStringHelper(this).add("Objects", objects).add("References", references)
					.add("Primitives", primitives).toString();
		}
	}
	 ...
	 * excluding static fields, {@code Class} objects, and fields defined in
	 * {@code enum}s (all these are considered shared values, which should not
	 * contribute to the cost of any single object graph), and any object for which
	 * the user-provided predicate returns {@code false}.
	 *
	 * @param rootObject
	 *          the root object of the object graph
	 * @param objectAcceptor
	 *          a predicate that returns {@code true} for objects to be explored
	 *          (and treated as part of the footprint), or {@code false} to forbid
	...
	public static Footprint measure(Object rootObject, Predicate<Object> objectAcceptor)
	{
		...
		Predicate<Chain> completePredicate = Predicates.and(ImmutableList.of(ObjectExplorer.notEnumFieldsOrClasses,
				Predicates.compose(objectAcceptor, ObjectExplorer.chainToObject), new ObjectExplorer.AtMostOncePredicate()));
